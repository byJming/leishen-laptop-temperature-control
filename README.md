...
- 保留手动满转风扇功能。
- 后台运行时支持 `Ctrl+Alt+Shift+F12` 切换手动满转，再按一次恢复自动温控。
- 支持停止接管并交还原厂默认控制。
...
后台守护进程运行时，可以按 `Ctrl+Alt+Shift+F12` 切换手动满转。这个快捷键由本项目注册，独立于原厂 `Fn+1` OSD；如果原厂热键只显示图标但不改变实际转速，请使用这个快捷键或窗口里的 `手动满转`。
...
- GPU >= 86°C 时三风扇直接 100%。
- SYS >= 80°C 时三风扇直接 100%。
- 升速较快，降速较慢，避免温度上来时响应迟钝，也避免转速频繁跳变。
